...
bot.start((ctx) => ctx.reply(`Привет, ${ctx.message.from.first_name}!\nНадеюсь ты не будешь кидать ничего из хуёвых каналов.`)) //ответ бота на команду /start

bot.on(['message', 'channel_post'], (ctx) => {
    if (ctx.message.hasOwnProperty('forward_from_chat') && (ctx.message.forward_from_chat.title.toLowerCase().includes("топор") || ctx.message.forward_from_chat.title.toLowerCase().includes("кб") )) {
    ...
    if (ctx.message.hasOwnProperty('text') ) {
        switch (ctx.message.text.toLowerCase()) {
            case 'во что поиграть':
                ctx.reply(`В ${GamesToPlay[Math.floor(Math.random()*GamesToPlay.length)]}`, { reply_to_message_id: ctx.message.message_id });
                break;
            case '🤡':
                ctx.reply(`${ctx.message.from.first_name}, сам ты клоун`, { reply_to_message_id: ctx.message.message_id });
                break;
            case 'да':
            ...
            case 'нет':
                ...
                break;
            case 'шлюхи аргумент':
                ctx.reply(`Аргумент не нужен, пидор обнаружен!`, { reply_to_message_id: ctx.message.message_id });
                break;
            case 'аргумент не вечен, пидор обеспечен':
                ctx.reply(`Пидор засекречен, твой анал не вечен)))))`, { reply_to_message_id: ctx.message.message_id });
                break;
            case 'пидор мафиозный, твой анал спидозный xd':
                ctx.reply(`Анал мой вечен, твой помечен)`, { reply_to_message_id: ctx.message.message_id });
                break;
            default:
                break;
        }
    ...
    if (ctx.message.hasOwnProperty('text') && ctx.message.text.toLowerCase().includes("халява") ) {
        epicFreeGames.getGames("US", true).then(async (res) => {
...
client.on('voiceStateUpdate', (oldState, newState) => {
    ...
    if (!(newState.channel && oldState.channel) && UpdatedChannel && UpdatedChannel.name == 'Основной') {
        ...
        nconf.load(function (err) {
                ...
                if (prevVoiceMembers.length > UpdatedChannel.members.size) {
                    prevVoiceMembers.forEach((VoiceUser) => {
                        if (!UpdatedChannel.members.has(VoiceUser.userId)) {
                            console.log(`${VoiceUser.nickname ? VoiceUser.nickname : VoiceUser.displayName} left`);
                            MovedUser = VoiceUser;
                        }
                    });
                } else if (prevVoiceMembers.length < UpdatedChannel.members.size) {
                    UpdatedChannel.members.forEach((VoiceUser, VoiceUserKey) => {
                        if (!prevVoiceMembers.some(prevVoiceUser => prevVoiceUser.userId == VoiceUserKey)) {
                            console.log(`${VoiceUser.nickname ? VoiceUser.nickname : VoiceUser.user.username} joined`);
                            MovedUser = VoiceUser;
                        }
                    });
            ...
            UpdatedChannel.members.forEach((VoiceUser) => {
                if (prevVoiceMembers && ( VoiceUser == MovedUser || VoiceUser.user.id == MovedUser.userId ) && prevVoiceMembers.length < UpdatedChannel.members.size) {
                    UsersInChannel += `➕ ${VoiceUser.nickname ? VoiceUser.nickname : VoiceUser.user.username}\n`;
                } else {
                    UsersInChannel += `${VoiceUser.nickname ? VoiceUser.nickname : VoiceUser.user.username}\n`;
                }
            });

            if (prevVoiceMembers && prevVoiceMembers.length > UpdatedChannel.members.size) {
                if (MovedUser.userId) {
            ...
            bot.telegram.sendMessage('-1001217699907', UsersInChannel, {parse_mode : 'HTML'}).then(function(msg) {
                if (prevVoiceMembers && prevVoiceMembers.length > UpdatedChannel.members.size && MovedUser.userId == '585411056088645642') {
                    bot.telegram.sendMessage('-1001217699907', '@asxok Сiбала', {parse_mode : 'HTML'});
                }
